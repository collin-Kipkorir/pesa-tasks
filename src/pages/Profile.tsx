    ...
    <div className="min-h-screen bg-muted/30 pb-24">
      <main className="mx-auto max-w-3xl space-y-5 px-4 py-5">
        <section className="rounded-2xl border bg-card p-5 shadow-[var(--shadow-card)]">
          ...
          {isAdmin && (
            <Link to="/admin" className="mt-3 inline-block text-xs font-semibold text-primary hover:underline">→ Admin Panel</Link>
          )}
        </section>

        <section className="rounded-2xl border bg-card p-5 shadow-[var(--shadow-card)]">
          <h2 className="text-base font-bold">Withdraw Funds</h2>
          <Input type="number" placeholder="Amount (KES)" value={amount} onChange={(e) => setAmount(e.target.value)} className="mt-4" />
          <Button variant="hero" size="lg" className="mt-4 w-full" onClick={handleWithdraw} disabled={submitting}>
            {submitting ? "Processing..." : "Withdraw"}
          </Button>
          {!user.activated && (
            <p className="mt-2 text-xs text-amber-700">Account not activated. KES {ACTIVATION_FEE} one-time fee required.</p>
          )}
        </section>
        ...
        <section className="rounded-2xl border bg-card p-5 shadow-[var(--shadow-card)]">
          ...
          {withdrawals.length === 0 ? (
            <p className="mt-6 text-center text-sm text-muted-foreground">No withdrawals yet.</p>
          ) : (
        ...
        <section className="rounded-2xl border bg-card p-5 shadow-[var(--shadow-card)]">
          ...
          <div className="mt-4 overflow-hidden rounded-lg border">
            <div className="grid grid-cols-3 bg-muted/60 px-4 py-2 text-xs font-semibold">
              <span>Amount</span><span className="text-center">Source</span><span className="text-right">Date</span>
            </div>
            {bonuses.length === 0 ? (
              ...
              bonuses.map((b, i) => (
                <div key={i} className="grid grid-cols-3 px-4 py-3 text-sm">
                  ...
                  <span className="text-right">{new Date(b.date).toLocaleDateString("en-GB", { month: "short", year: "numeric" })}</span>
                </div>
              ))
            )}
          </div>
        ...
        <Button variant="hero" size="lg" className="w-full" onClick={() => { logout(); navigate("/login"); }}>Log Out</Button>
      </main>

...
function TaskInner() {
  ...
  useEffect(() => {
    if (!loading && !survey) navigate("/dashboard");
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-5 bg-muted/30 px-6 text-center">
        ...
        <div className="flex gap-3">
          ...
          <Button variant="hero" onClick={() => navigate("/dashboard")}>More Surveys</Button>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-muted/30 pb-12">
      <header className="sticky top-0 z-20 border-b bg-card">
        <div className="mx-auto flex max-w-2xl items-center gap-3 px-4 py-3">
          <Link to="/dashboard" className="rounded-md p-1.5 hover:bg-muted"><ArrowLeft className="h-5 w-5" /></Link>
          <div className="flex-1">
            <h1 className="text-sm font-bold leading-tight">{survey.title}</h1>
            <p className="text-[11px] text-muted-foreground">Question {step + 1} of {total} · Reward KES {survey.reward}</p>
          </div>
        </div>
        ...
        <article className="rounded-2xl border bg-card p-6 shadow-[var(--shadow-card)] animate-in fade-in slide-in-from-right-4">
          ...
          <div className="mt-5 space-y-2">
            {current.options.map((opt, i) => {
              ...
              return (
                <button key={i} onClick={() => setAnswers({ ...answers, [step]: opt })}
                  className={`flex w-full items-center justify-between rounded-lg border-2 px-4 py-3 text-left text-sm font-medium transition-all ${selected ? "border-primary bg-primary/5 text-foreground" : "border-input bg-background hover:border-primary/40"}`}>
                  <span>{opt}</span>
          ...
          <div className="mt-6 flex gap-3">
            {step > 0 && <Button variant="outline" onClick={() => setStep(step - 1)} className="flex-1">Previous</Button>}
            <Button variant="hero" className="flex-1" onClick={next} disabled={!answered || submitting}>
              {submitting ? "Submitting..." : step === total - 1 ? `Finish & Claim KES ${survey.reward}` : "Next"}
            </Button>
          </div>
        </article>
      </main>
...
export default function Task() {
  return <RequireAuth><TaskInner /></RequireAuth>;
}

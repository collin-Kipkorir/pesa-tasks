    ...
    <div className="min-h-screen bg-muted/30 pb-24">
      ...
      <main className="mx-auto max-w-3xl space-y-5 px-4 py-5">
        <section className="rounded-2xl border bg-card p-5 shadow-[var(--shadow-card)]">
          ...
          <p className="mt-3 text-center text-2xl font-bold text-primary">KES {(user?.balance || 0).toLocaleString()}</p>
          <Button variant="hero" size="lg" className="mt-4 w-full" onClick={handleWithdraw}>
            <ArrowDown className="mr-2 h-4 w-4" />Withdraw Funds
          </Button>
        </section>
          ...
          <div className="mt-4 h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={weekly} margin={{ top: 8, right: 8, left: -16, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="value" stroke="oklch(0.5 0.15 145)" strokeWidth={2} fill="url(#walletFill)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </section>

        {rewards.length === 0 && <p className="text-center text-sm text-muted-foreground">No transactions yet.</p>}
      </main>
...
export default function Wallet() {
  return <RequireAuth><WalletInner /></RequireAuth>;
}
